...
{
    protected $signature = 'canvas:generate {model} {--force} {--admin}';
    ...
    private function insertNavLink()
    {
        ...
        if ($this->option('admin'))
            $nav_file = resource_path('views/vendor/canvas/admin/nav.blade.php');
        else
            $nav_file = resource_path('views/layouts/app.blade.php');

        if (file_exists($nav_file)) {
            $nav_contents = file_get_contents($nav_file);
            $nav_hook = '          <!-- insert meus above here-->';
    ...
    {
        if ($this->option('admin'))
            $stub_contents = $this->replace(rtrim(file_get_contents(__DIR__ . '/../../resources/stubs/generate/routes-admin.stub')));
        else
